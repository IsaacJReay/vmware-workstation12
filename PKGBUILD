...
sha256sums=(
  ...
  '6f2789b252d6d29339bfd844e6d0a5d83e3071509bc12a3927e2fd65b852bdd2'
  'c2e1affde7d8786493b5bf04e7a7bd378651544ad7491d980fdc851c83124121'

  'a69f10256a3a5cd631aee4d9e068357684384e33ffee3622f777e3f3fba536e7'
)
...
if [ -z "$_remove_vmware_keymaps_dependency" ]; then
  ...
  sed -i 's,@@BINARY@@,pkexec env DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY HOME=$HOME USER=$USER GTK_PATH=:/usr/lib/gtk-2.0 /usr/bin/vmware,' "$pkgdir/usr/share/applications/vmware-workstation.desktop"
  sed -i 's,@@BINARY@@,pkexec env DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY HOME=$HOME USER=$USER GTK_PATH=:/usr/lib/gtk-2.0 /usr/bin/vmplayer,' "$pkgdir/usr/share/applications/vmware-player.desktop"
  sed -i 's,@@BINARY@@,pkexec env DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY HOME=$HOME USER=$USER GTK_PATH=:/usr/lib/gtk-2.0 /usr/bin/vmware-netcfg,' "$pkgdir/usr/share/applications/vmware-netcfg.desktop"
...
if [ -n "$_enable_macOS_guests" ]; then
  ...
  install -Dm644 "${srcdir}/vmware.conf" "${pkgdir}/etc/modules-load.d/vmware.conf"
}
